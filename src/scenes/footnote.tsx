...
export default function makeFootnote(num: number) {
  return makeScene2D(function* (view) {
    ...
    yield* all(
      ...
      refs.bar[1]().to([footnote.width / 2, footnote.y + footnote.height], 0.4),
      refs.notch[0]().from(
        [-(footnote.width / 2) - footnote.lineWidth / 2, 0],
        0.4
      ),
      refs.notch[0]().to([-(footnote.width / 2) + footnote.notchWidth, 0], 0.4),
      refs.notch[1]().from(
        [footnote.width / 2 + footnote.lineWidth / 2, 0],
        0.4
      ),
      refs.notch[1]().to([footnote.width / 2 - footnote.notchWidth, 0], 0.4),
      refs.notch[2]().from(
        [-(footnote.width / 2) - footnote.lineWidth / 2, 0],
        0.4
      ),
      refs.notch[2]().to([-(footnote.width / 2) + footnote.notchWidth, 0], 0.4),
      refs.notch[3]().from(
        [footnote.width / 2 + footnote.lineWidth / 2, 0],
        0.4
      ),
      refs.notch[3]().to([footnote.width / 2 - footnote.notchWidth, 0], 0.4),
